...
import { callAPI, getSupabaseConfig, saveSession } from '../services/api';
...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess }) => {
  ...
  const handleSupabaseLogin = async () => {
    ...
    if (!url || !anonKey) {
      setErrorMsg('Supabase belum dikonfigurasi. Isi VITE_SUPABASE_URL dan VITE_SUPABASE_ANON_KEY di file .env.');
      return;
    }
    ...
    try {
      ...
      if (!supabase) {
        setErrorMsg('Supabase client belum siap. Cek konfigurasi environment Anda.');
        return;
      }
      ...
      if (error) {
        setErrorMsg(error.message || 'Login Supabase gagal.');
        return;
      }
      ...
      setErrorMsg('Login Supabase gagal. Periksa konfigurasi OAuth di Supabase.');
    } finally {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-2xl border border-slate-100 space-y-6">
          ...
          >
            ...
            <span>{googleLoading ? 'Menghubungkan Supabase...' : 'Masuk dengan Google (Supabase)'}</span>
          </button>
