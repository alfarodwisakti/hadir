...
index c7d237b7bbb7445c3d34670033d9f72f3bb85f71..9f90f6a96b3d74d40f2d25f0eca2dffefc6ee9bb 100644
...
+    return Array.isArray(parsed) ? parsed.map(normalizeAdminUser).filter((user) => user.username && user.password) : [];
   } catch {
     return [];
   }
 }
...
+      nama: user.nama.trim() || user.username.trim(),
+      role: user.role.trim() || "Admin"
+    }));
+
+  localStorage.setItem("presensi_local_admin_users", JSON.stringify(users));
 }
...
+function normalizeAdminUser(user: any): AdminUser {
+  return {
...
-            role: String((user.role || user.Role) ?? "Admin").trim() || "Admin"
-          }))); 
+        if (action === "getAdminUsers" && Array.isArray(json.data)) {
+          saveLocalAdminUsers(json.data.map(normalizeAdminUser));
         }
       }
       return json;
-    } else {
...
+    // Login offline is allowed only after the Admin sheet has been synchronized.
+    // This prevents an empty cache from being reported as a valid Admin source.
     if ((action === "login" || action === "getAdminUsers") && getLocalAdminUsers().length === 0) {
       return adminSyncUnavailableResponse();
     }
     return executeLocalAction(action, payload);
